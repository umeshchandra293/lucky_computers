  ...
  return (
    <>
      ...
      <motion.header
        ...
        className="fixed left-0 top-4 z-50 w-full px-4 md:px-8"
      >
        ...
        <nav className="mx-auto flex w-full max-w-6xl items-center justify-between rounded-full border border-white/60 bg-white/60 px-4 py-2.5 shadow-[0_8px_30px_rgba(37,99,235,0.08)] backdrop-blur-xl">
          ...
          <a href="#home" className="flex shrink-0 items-center gap-3">
            <img
              ...
              className="h-10 w-10 rounded-full object-cover shadow-sm ring-2 ring-white/70"
            />
            <span className="hidden flex-col leading-none sm:flex">
              <span className="font-black tracking-tight text-slate-900">LUCKY COMPUTERS</span>
              <span
                className="-mt-0.5 text-sm text-blue-600"
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="absolute left-4 right-4 top-20 z-40 mx-auto max-w-sm rounded-2xl border border-white/60 bg-white/80 p-4 shadow-2xl backdrop-blur-xl md:hidden"
            >
              <div className="flex flex-col gap-2">
                {NAV_LINKS.map((link) => (
                  <a
                    ...
                    className="rounded-xl px-4 py-3 text-sm font-bold uppercase tracking-widest text-slate-600 transition-colors hover:bg-blue-50 hover:text-blue-600"
                  >
                    {link.name}
                  </a>
                ))}
                ...
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={() => setIsOpen(false)}
                  className="group relative flex w-full items-center justify-between overflow-hidden rounded-xl bg-slate-900 py-2 pl-4 pr-1.5 text-sm font-bold uppercase tracking-widest text-white transition-colors hover:bg-blue-600"
                >
